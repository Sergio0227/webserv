...
void Brain::parseLocation(size_t *i, int server_index, std::string location_name)
{
	...
	while (this->_config_files[server_index][++(*i)] != "}")
	{
		...
		if (param == "allow_methods")
			ref_loc.setAllowedMethods(value, static_cast<char>(server_index));
		else if (param == "root")
			ref_loc.setRoot(value, static_cast<char>(server_index));
		else if (param == "index")
			ref_loc.setIndex(value, static_cast<char>(server_index));
		else if (param == "autoindex")
			ref_loc.setAutoindex(value, static_cast<char>(server_index));
		else if (param == "alias")
			ref_loc.setAlias(value, static_cast<char>(server_index));
		else if (param == "return")
			ref_loc.setReturnValue(value, static_cast<char>(server_index));
		else if (param == "client_max_body_size")
			ref_loc.setClientMaxBodySize(value, static_cast<char>(server_index));
	}
